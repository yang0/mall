...
FieldInfo = namedtuple('FieldInfo',
    'name type_code display_size internal_size precision scale null_ok comment')
...
class DatabaseIntrospection(BaseDatabaseIntrospection):
    ...
    def get_table_description(self, cursor, table_name):
        ...
        cursor.execute("""
            ...
            WHERE table_name = %s AND table_schema = DATABASE()
                """, [table_name])
        comment_map = dict(cursor.fetchall())
        ...
        result =  [FieldInfo(*((force_text(line[0]),)
                            + line[1:3]
                            + (length_map.get(line[0], line[3]),)
                            + numeric_map.get(line[0], line[4:6])
                            + (line[6],)
                            + (comment_map.get(line[0], line[3]),)
                            ))
